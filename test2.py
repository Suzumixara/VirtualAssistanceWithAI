...
class VoiceAssistantCore:
    def __init__(self):
        ...
        # Пути к программам (можно изменить под свою систему)
        self.PROGRAMS = {
        ...
        # Hugging Face API
        self.HF_API_TOKEN = "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"
        self.HF_API_URL = "https://api-inference.huggingface.co/models/bigscience/bloomz"
    ...
    def execute_system_command(self, command: str) -> Optional[str]:
            ...
            if program_name in command:
                ...
                try:
                    if program_name == "проводник":
                        subprocess.Popen([path])  # Для проводника без параметров
                    else:
                        os.startfile(path)
        ...
        # Системные команды (оставьте существующие)
    ...
    def ask_ai(self, prompt: str) -> str:
        ...
        try:
            ...
            print("Тело ответа API:", response.text)  # ← вот это покажет точную ошибку
